...
pub fn create_trade_operation<'info>(
    ...
    let vault_identifier = params.vault_identifier;
    ...
    let trade_type: TradeType = match creator_accounts {
        ...
        (Ok(creator_send_account), Ok(creator_receive_account)) => match mints_address {
            (Some(Ok(mint_token_creator_trade)), Some(Ok(mint_token_creator_receive))) => {
                ...
                ctx.accounts
                    .transfer_token_to_vault(params.exchange_amount)?;
        ...
        (Err(_), Ok(creator_receive_account)) => match mints_address {
            (Some(Ok(mint_token_creator_receive)), None) => {
                ...
                ctx.accounts
                    .create_native_account_vault(vault_identifier, params.order_id)?;
                // transfer SOL -> Vault Escrow
                ctx.accounts.transfer_native_vault(params.exchange_amount)?;
    ...
    ctx.accounts.escrow_state.exchange_amount = params.exchange_amount;
    ...
    ctx.accounts.escrow_state.vault_identifier = vault_identifier;
...
impl<'info> Create<'info> {
    ...
    fn create_native_account_vault(&self, vault_identifier: u8, order_id: u64) -> Result<()> {
        let creator_key = self.creator.key();
        let order_id_bytes = order_id.to_le_bytes();
        let vault_seed = &[&[
            VAULT_PDA_SEED,
