...
#[derive(Accounts)]
#[instruction(order_id: u64, state_bump: u8, vault_identifier: u8)]
pub struct Exchange<'info> {
    ...
    #[account(mut)]
    pub recipient_sender_account: AccountInfo<'info>,
...
pub fn handler_exchange(ctx: Context<Exchange>) -> Result<()> {
    ...
    let exchange_amount = ctx.accounts.escrow_state.exchange_amount;
    ...
    match trade_type {
        ...
        Ok(TradeType::TokenToken) => {
            // convert `recipient_sender_account` to TokenAccount
            let partner_token_accounts: (TokenAccountType, TokenAccountType) = (
                Account::try_from(&ctx.accounts.recipient_sender_account),
            ...
            match partner_token_accounts {
                (Ok(partner_send_token_account), Ok(partner_receive_token_account)) => {
                    ...
                    // make sure `recipient_sender_account` is associated token account with `creator_receive_token_mint`
        ...
        Ok(TradeType::SolToken) => {
            ...
            ctx.accounts.transfer_to_partner_native(exchange_amount)?;
...
impl<'info> Exchange<'info> {
    fn transfer_to_partner_native(&self, amount: u64) -> Result<()> {
        ...
        let vault_identifier = self.escrow_state.vault_identifier;
        let vault_seed = &[&[
            VAULT_PDA_SEED,
            creator_key.as_ref(),
            order_id_bytes.as_ref(),
            bytemuck::bytes_of(&vault_identifier),
        ][..]];
    ...
    fn send_tokens_to_target(&self, amount: u64) -> Result<()> {
        let creator_key = self.creator.key();
        let order_id_bytes = self.escrow_state.order_id.to_le_bytes();
    ...
    fn transfer_to_creator_token(&self, amount: u64) -> Result<()> {
        // transfer token partner -> creator
        transfer_token_to_account(
            self.recipient_sender_account.to_account_info(),
    ...
    fn finalize_token_vault(&self) -> Result<()> {
